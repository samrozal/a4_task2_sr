...
# 1. Introduction
This report provides an exploratory overview into differences in weights and sizes of juvenile snowshoe hares observed at the Bonanza Creek Long Term Ecological Research site (a portion of the US Long Term Ecological Research Network) between the years of 1998 and 2012. Specifically, we analyze the differences in juvenile snowshoe hare weights between sexes (male and female) and experimental sites (Bonanza Riparian, Bonanza Mature, and Bonanza Black Spruce). We also farther explore the relationship between juvenile hare weight and juvenile hare hind foot length by building a simple linear regression model to explore this relationship 
...
There is a significant linear relationship in which juvenile hare weight can be predicted by juvenile hare hind foot length (p=`r round(juvenile_hares_lm_tidy$p.value[1], 2)`, R^2^ =`r round(juvenile_hares_lm_glance$r.squared[1], 3)`). The slope value of $\beta$ = `r round(juvenile_hares_lm_tidy$estimate[2], 2)` mm/g  indicates a predicted increase of `r round(juvenile_hares_lm_tidy$estimate[2], 2)` g in juvenile snowshoe hare weight for a 1 millimeter increase in hind foot length. There is a moderate, positive, significant correlation between juvenile snowshoe hare hind foot length and weight (Pearson's *r* = `r round(juvenile_hares_cor_tidy$estimate,2)`, p= `r (juvenile_hares_cor_tidy$p.value)`). Undisplayed diagnostic plots indicate that the distribution of the residual is hetroscedastic and non-normal, which indicates that although there is a significant linear relationship, the linear model may not be the best fit or important varaibles are missing from the current model.  
...
Future analysis surrounding this topic should include:
 ...
 - Constructing a multivariate linear regression model to account for potentially signifcant varaibles that were not included in the curent simple linear regression model. 
